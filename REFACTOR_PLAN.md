...
- [x] Phase 8 — Admin, Analytics
- [x] Phase 9 — Dọn dẹp & nghiệm thu

> **Ghi chú Phase 9:** `SeedController` cố tình giữ truy cập `ISignMateDbContext` trực tiếp —
> đây là tiện ích dev/ops gọi `DatabaseSeeder` (nằm ở Infrastructure) để nạp dữ liệu mẫu,
> không phải endpoint nghiệp vụ của app, nên không đưa vào khuôn CQRS. Toàn bộ controller
> nghiệp vụ còn lại đều "siêu mỏng" (chỉ `Mediator.Send` + bọc `ApiResponse`), Application
> không còn business Service layer (thư mục `Services` rỗng), `Program.cs`/DI sạch.
